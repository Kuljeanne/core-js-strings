...
function removeFirstOccurrences(str, value) {
  ...
  return indexStart + 1
    ? str.substring(0, indexStart) + str.substring(indexStart + value.length)
    : str;
}
...
function isPalindrome(str) {
  const strWithNoSpace = str.replaceAll(/[\s.,!?]/g, '').toLowerCase();

  for (let i = 0; i < strWithNoSpace.length / 2; i += 1) {
    if (strWithNoSpace[i] !== strWithNoSpace[strWithNoSpace.length - 1 - i]) {
      return false;
    }
  }
  return true;
...
function reverseWords(str) {
  ...
  const res = [];
  for (let i = 0; i < words.length; i += 1) {
    res.push(reverseString(words[i]));
  }
  return res.join(' ');
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (str[i] === ' ' || str[i] === '!' || str[i] === '?') {
      ...
      const firstBig = 'A'.charCodeAt(0);
      const firstSm = 'a'.charCodeAt(0);
      const first = str[i].toLowerCase() === str[i] ? firstSm : firstBig;
      const code = first + ((str.charCodeAt(i) - first + 13) % 26);
